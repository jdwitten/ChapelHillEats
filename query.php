...
if($prev){$sql = "select rid,rname,phone,address,rating from acsm_2fd6830ca4dfb48.restaraunts where ".$typeEval . $locationEval . $priceEval . $ratingEval;}
...
function buildRestaurant($rname, $phone, $address, $rating,$rid){
       echo "<a href = chapelhilleats.azurewebsites.net/restaurantInfo.php?rname=".$rid."><div class = 'restaurant' ><span><div class = 'name'>".$rname."</div><div class = 'rating'>Rating: ".$rating."</div><div class = 'info'>Phone Number: ".$phone."</div><div class = 'info'>Address: ".$address."</div></span></div></a>";
}
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()){ 
        ...
        */
        ...
        buildRestaurant($row['rname'],$row['phone'],$row['address'],$row['rating'],$row['rid']);
    }
    echo "</table>";
} else {
    echo "0 results";
